...
void Player::ChangeDir(std::optional<model::Direction> dir) {
...
void Player::SetSpeed(Dog::Speed speed) {
    dog_->SetSpeed(speed);
}

void Player::Move(std::chrono::milliseconds time) {
...
int64_t Player::FindRoadIndex(model::Dog::Coordinate pos, std::unordered_set<size_t>& viewed_road) {
    const auto& roads = game_->GetMap()->GetRoads();
...
std::pair<Player*, Players::Token> Players::Add(Dog* dog, GameSession* session) {
...
void Players::MovePlayers(std::chrono::milliseconds time) {
    for (const auto& player : players_) {
        player->Move(time);
    }
}

std::vector<Player*> Players::GetAllPlayers() const {

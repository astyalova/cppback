...
public:
    ...
    int GetLootValue(MapId id, size_t type_index) const;
    const std::vector<model::LootType>& GetLootTypesData(MapId id) const;
...
private:
    ...
    std::unordered_map<MapId, std::vector<model::LootType>, util::TaggedHasher<MapId>> loot_types_data_;
